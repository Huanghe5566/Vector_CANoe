...
namespace CANalyzerCANoeCLibrary
{
  ...
  int VValueEntity::GetInteger(long long* x) const
  {
    ...
    if (mPort->GetValue(mTypeLevel, mMemberHandle, mValueClass, &(fbValue.mVIAObject)) != kVIA_OK)
      return CCL_VALUENOTACCESSIBLE;
  ...
  int VValueEntity::IsValid(bool* isValid) const
  {
    ...
    if (mPort->GetValue(VIAFbTypeLevel::eImpl, kVIAFbTypeMemberHandleWhole, mValueClass, &(fullValue.mVIAObject)) != kVIA_OK)
      return CCL_VALUENOTACCESSIBLE;
